#!/usr/bin/env bash
# =============================================================================
# Issuer Web VS — Local Setup (Verana v0.10.1+, devnet only)
# =============================================================================
#
# This script sets up the Issuer Web VS Agent locally (child service):
#   1. Creates issuer-web-vs's own Corporation and grants it operator
...
#   - MNEMONIC (or ISSUER_WEB_VS_MNEMONIC) env var — a funded devnet account
#
# Usage:
#   source issuer-web-vs/config.env
#   MNEMONIC="..." ./issuer-web-vs/scripts/setup.sh
#
# =============================================================================
...
USER_ACC="${USER_ACC:-issuer-web-vs-devnet-admin}"
OUTPUT_FILE="${OUTPUT_FILE:-${SERVICE_DIR}/ids.env}"
MNEMONIC="${MNEMONIC:-${ISSUER_WEB_VS_MNEMONIC:-}}"
...
if [ -n "$MNEMONIC" ]; then
  ...
  err "Export MNEMONIC (or ISSUER_WEB_VS_MNEMONIC) with a funded devnet account and re-run."
  exit 1
fi
...
if [ -n "${CORPORATION_ID:-}" ]; then
  ...
  create_corporation "did:example:issuer-web-vs-${CHAIN_ID}" "$EGF_DOC_URL" "" \
    "$OA_MSGS_ISSUER"
  ok "Corporation created: CORPORATION_ID=$CORPORATION_ID — add this to issuer-web-vs/config.env to skip next time"
fi
